...
A multi-tenant property-management SaaS — Next.js frontend, .NET 10 API, PostgreSQL + Keycloak + RabbitMQ + Redis, deployed to DigitalOcean Kubernetes via Helm with full observability and CI/CD.
...
| The prod (DOKS + Helm + Terraform) deployment | [`docs/architecture/deployment-prod.md`](./docs/architecture/deployment-prod.md) |
...
| The reasoning behind a specific call | [`docs/architecture/adr/`](./docs/architecture/adr/) (8 ADRs) |

## Quick start (local dev)

```bash
# 17 services on a single bridge network:
docker compose up -d
...
- Operations runbooks (migrations, Nginx SSL, security hardening, Terraform) — see [`docs/operations/`](./docs/operations/).
