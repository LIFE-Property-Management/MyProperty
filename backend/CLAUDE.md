...
- **`RequireTenant` endpoints:** `TenantController` (`/api/v1/tenant/*`) owns the tenant-portal surface — `GET /tenant/lease` (the caller's active lease: `200` `TenantLeaseDto` or `204`) and `POST /tenant/lease/cancel` (immediate self-service cancellation: `Lease.Terminate()` + a landlord notification email enqueued via `IBackgroundJobQueue`, dashboard cache invalidated; `404` when there's no active lease). `MeController` (`/api/v1/me`) keeps **identity only** (the lazy `User`-row upsert + roles echo) — the lease read used to live there and was moved here (organize by consumer). Notice period / penalty / deposit on cancellation are deferred (decision D2).
...
- **Endpoints** (all under `/api/v1/invites`):
    ...
    - `GET /by-token/{token}` — anonymous preview. Returns `200` with a `status` field (`InviteStatus`, serialized as a string) for **any resolved invite**, so the accept page can render a status-specific view (Accepted/Rejected/Expired). Returns `404` **only** for a truly unknown token-hash (the token is a secret bearer — don't confirm existence). A `Pending` invite past `ExpiresAt` (not yet swept by the hourly job) is reported as `Expired`.
    - `POST /{token}/accept` — anonymous **new-user** path. Provisions a Keycloak account + `User` row, then creates the `Lease` and marks invite `Accepted` in a single unit of work. If an account already exists for the invite email, returns `409` ("…already exists. Please log in instead.") so a new-user form can't hijack an existing account — the user is steered to the authenticated `claim` path.
    - `POST /{token}/claim` — authenticated **returning-tenant** path (`RequireTenant`, `authenticated` rate limit). Reuses the logged-in account (no Keycloak provisioning); the JWT-resolved email must match the invite email (case-insensitive) — mismatch is `403` with `"This invite was sent to a different email address."`. Creates the `Lease` and marks invite `Accepted`.
    - `POST /{token}/reject` — anonymous. Marks invite `Rejected`. Returns 204.
    - `GET /` — landlord-only (`RequireLandlord`, `authenticated` rate limit). Paginated list of the caller's own invites (`PagedResult<InviteListItemDto>`), newest first, optional `?status=` filter (`Pending`/`Accepted`/`Rejected`/`Expired`/`Revoked`). Drives the dedicated `/dashboard/invites` management page.
    - `POST /{id}/revoke` — landlord-only. Cancels one of the caller's own invites; only a `Pending` or `Expired` invite can be revoked (→ `Revoked`), else `409`. `403` if the invite belongs to another landlord. Returns 204.
    - `POST /{id}/resend` — landlord-only. Re-issues a `Pending`/`Expired` invite: a **fresh token** is generated (the previously-mailed link stops resolving — `404` on preview), `ExpiresAt` resets to `now + ExpiryDays`, status returns to `Pending`, and the invite email is re-enqueued. Returns `200` with `InviteResentDto { InviteId, ExpiresAt }`. `403` for another landlord's invite; `409` for an Accepted/Rejected/Revoked invite.
- **`InviteStatus.Revoked = 4`** distinguishes a landlord-cancelled invite from a naturally `Expired` one in the invite log. Additive enum value; stored as the string `"Revoked"` in the existing 16-char `Status` varchar (`HasConversion<string>`), so **no migration** is required.
- **No 410 Gone.** `accept`/`reject` still return 404 for any invite that is not `Pending` or is past `ExpiresAt`. **Preview is the exception** (status-disclosure rule above): a resolved non-Pending/expired invite returns `200`-with-status, not `404`.
- **`InviteAccepted` / `InviteRejected` integration events** are published **after commit** by the accept/claim/reject handlers (`invite.accepted` / `invite.rejected` routing keys). Consumers push a SignalR signal to `landlord:{LandlordId}`; the accepted consumer also enqueues a landlord notification email. See the Message Queue + Real-time sections.
- **Token + invite-email construction is shared** between `CreateInviteHandler` and `ResendInviteHandler` via `InviteTokenFactory.Issue()` (plain token + hash) and `InviteEmailFactory.Build(...)` (email body + CTA link). This removed the duplicated token-generation/hashing that previously lived inline in `CreateInviteHandler`.
- **Per-property occupancy.** The landlord property list (`PropertyDto`) and detail (`PropertyDetailDto`) carry `HasActiveLease` + `HasPendingInvite` (two bools) so the UI can render Vacant / Invite-pending / Leased. "Pending invite" is effective-pending (`Pending` **and** not past `ExpiresAt`). The list computes both via set-based batch repo helpers (`ILeaseRepository.GetActiveLeaseIdsByPropertyAsync`, `IInviteRepository.GetPendingInviteIdsByPropertyAsync`) to avoid an N+1. To drive the landlord "Cancel lease" / "Cancel invitation" actions without a second round-trip, both DTOs also carry the relevant ids: the list `PropertyDto` carries `ActiveLeaseId` (`Guid?`, the Active lease's id when `HasActiveLease`, else null — unambiguous by the single-active-lease-per-property invariant) and `PendingInviteId` (`Guid?`, the pending invite's id when `HasPendingInvite`, else null); the detail `PropertyDetailDto` carries `PendingInviteId` and each tenant row (`PropertyTenantDto`) carries its `LeaseId`.
  - **⚠️ TODO (guard rail) — `PendingInviteId` assumes one pending invite per property, which is NOT yet enforced.** A property can currently hold several `Pending` invites (see the "Still permissive (by design)" follow-up below), so `GetPendingInviteIdsByPropertyAsync` / the detail query return the **most recent** effective-pending invite as "the" one. The frontend three-state UX (D7) treats a property as having a single pending invite. Before relying on this as an invariant, add the **one-pending-invite-per-property guard** in `CreateInviteHandler` (reject creating a second effective-pending invite → `409`) and update this note. A DB partial unique index is *not* a clean fit here (the `Status='Pending'` predicate can't express the `ExpiresAt > now` effective-pending semantics, and an expired-but-unswept row would falsely occupy the slot) — enforce in the handler.
...
- ~~A property could end up with **multiple active leases**: a stale `Pending` invite could be accepted/claimed onto a property that has since been leased (`AcceptInviteHandler`/`ClaimInviteHandler` had no occupancy guard).~~ **FIXED.** Both accept handlers now reject with `409 ("This property already has an active lease.")` when `ILeaseRepository.HasActiveLeaseForPropertyAsync` is true (checked before Keycloak provisioning on the new-user path), and a **partial unique index** `Lease(PropertyId) WHERE Status = 'Active' AND DeletedAt IS NULL` enforces it at the database against the accept/accept race (migration `AddSingleActiveLeasePerPropertyIndex`). Terminated/Expired leases don't count, so re-letting a vacated property still works.
  - **Still permissive (by design):** `CreateInviteHandler` does **not** block creating multiple `Pending` invites for one property — courting several candidates is a legitimate funnel, and the acceptance-time guard + unique index are the backstop (the first accept wins; later ones get `409`). D7's UI ("Invitation pending"/"Cancel lease" instead of "Add lease") discourages accidental duplicates. If a strict one-pending-invite-per-property rule is ever wanted, add the guard in `CreateInviteHandler`.
  - **Note:** this is the per-**property** invariant. The per-**tenant** "single active lease" invariant is still deferred — see the "(For Later)" section below.
- ~~RabbitMQ `InviteAccepted` / `InviteRejected` event publishing (M3.8).~~ **DELIVERED.** Published after commit by the accept/claim/reject handlers; consumers in `Infrastructure/Messaging/Consumers/`.
- ~~SignalR push to landlord on accept/reject (M3.6).~~ **DELIVERED.** `InviteAcceptedConsumer`/`InviteRejectedConsumer` push to `landlord:{LandlordId}` via `INotificationDispatcher` (`"InviteAccepted"`/`"InviteRejected"`); accepted also enqueues a landlord email.
