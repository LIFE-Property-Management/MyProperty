...
- **`RequireTenant` endpoints:** `TenantController` (`/api/v1/tenant/*`) owns the tenant-portal surface — `GET /tenant/lease` (the caller's active lease: `200` `TenantLeaseDto` or `204`) and `POST /tenant/lease/cancel` (immediate self-service cancellation: `Lease.Terminate()` + a landlord notification email enqueued via `IBackgroundJobQueue`, dashboard cache invalidated; `404` when there's no active lease). `UsersController` (`/api/v1/me`) keeps **identity only** (the lazy `User`-row upsert + roles echo) — the lease read used to live there and was moved here (organize by consumer). Notice period / penalty / deposit on cancellation are deferred (decision D2).
