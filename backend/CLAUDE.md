...
- **Per-property occupancy.** The landlord property list (`PropertyDto`) and detail (`PropertyDetailDto`) carry `HasActiveLease` + `HasPendingInvite` (two bools) so the UI can render Vacant / Invite-pending / Leased. "Pending invite" is effective-pending (`Pending` **and** not past `ExpiresAt`). The list computes both via set-based batch repo helpers (`ILeaseRepository.GetActiveLeaseIdsByPropertyAsync`, `IInviteRepository.GetPropertyIdsWithPendingInviteAsync`) to avoid an N+1. To drive the landlord "Cancel lease" action (`PATCH /leases/{id}/terminate`) without a second round-trip, the list `PropertyDto` also carries `ActiveLeaseId` (`Guid?`, the Active lease's id when `HasActiveLease`, else null — unambiguous by the single-active-lease-per-property invariant) and each detail-DTO tenant row (`PropertyTenantDto`) carries its `LeaseId`.
