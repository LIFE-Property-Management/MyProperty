...
public sealed class InviteRejectedConsumer : BackgroundService
{
    private const string ExchangeName = "myproperty";
    private const string QueueName    = "myproperty.invite.rejected";
    private const string RoutingKey   = "invite.rejected";

    private readonly IConnectionFactory _factory;
    private readonly ILogger<InviteRejectedConsumer> _logger;

    public InviteRejectedConsumer(
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            try
            {
                var evt = JsonSerializer.Deserialize<InviteRejectedEvent>(ea.Body.Span);
                if (evt is null)
                {
                    _logger.LogWarning("Failed to deserialize InviteRejectedEvent — discarding");
                    await channel.BasicNackAsync(ea.DeliveryTag, multiple: false, requeue: false);
                    return;
                }

                _logger.LogInformation(
                    "InviteRejected received — InviteId {InviteId}, LandlordId {LandlordId} (SignalR push deferred to M3.6)",
            ...
            {
                _logger.LogError(ex, "Error processing invite.rejected message");
                await channel.BasicNackAsync(ea.DeliveryTag, multiple: false, requeue: false);
            }
        };
