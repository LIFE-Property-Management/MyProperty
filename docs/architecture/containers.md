...
4. **The receipt-OCR path has a runtime kill-switch.** The self-hosted Unleash server gates it via `payments.ocr-autoextract` (default on, fail-open); when off, submitted receipts stay manual-entry and no Anthropic call is made. See [ADR-0010](./adr/0010-unleash-for-feature-flags.md) and the OCR sequence in [`data-flow.md`](./data-flow.md).
...
| **API + Hub + Workers** | .NET 10 ASP.NET Core (chiseled Ubuntu 24.04 distroless), EF Core 10, FluentValidation 12, Asp.Versioning 10, Serilog (CLEF→stdout, scraped by Promtail), prometheus-net 8, Hangfire 1.8, SignalR + Redis backplane, MailKit 4, Anthropic Messages API (HttpClient, no SDK), Unleash.Client | REST API (`/api/v1/*`), real-time hub (`/hubs/notifications`), Hangfire job server, Hangfire dashboard (`/hangfire`, Admin-gated) | Clean Architecture in one process keeps deploy simple at MVP scale; ADRs cover the substantive picks. |
| **AIOps Webhook** | Python 3.14-slim (non-root), FastAPI, anthropic SDK, httpx | Receives Alertmanager firing alerts → triages via Claude Haiku → posts to **Discord** (`#alerts`) as rich embeds | Light, single-purpose, isolates the AI dependency from the API process. Graceful degradation if the AI key or Discord webhook URL is missing. |
| **n8n** | n8n 1.76 (self-hosted, SQLite store on a volume) | Self-hosted workflow automation (M5.8) — tenant-inquiry triage: webhook → Claude Haiku → **Discord**. The workflow is imported + activated on first boot by the `n8n-init` sidecar | Visual, low-code automation for a product event flow without a bespoke service; isolates the integration from the API. Graceful degradation if the AI key or Discord webhook URL is missing. |
...
| **Unleash** | Unleash OSS server (Node), reusing the shared Postgres (`unleash` DB) | Self-hosted feature flags (M5.6); the backend SDK polls a cached snapshot. First flag `payments.ocr-autoextract` is the receipt-OCR kill-switch. ClusterIP only — no public ingress in prod | Self-hosted avoids per-seat pricing; a runtime kill-switch without a redeploy — [ADR-0010](./adr/0010-unleash-for-feature-flags.md). |

## Containers — data + brokers

| Container | Tech / Version | Purpose | Why this choice |
|---|---|---|---|
| **PostgreSQL** | PostgreSQL 16-alpine — self-hosted StatefulSet in **both** dev and prod (the DOKS managed-DB path is retired — [ADR-0009](./adr/0009-hetzner-project-02-over-doks.md)) | App + Hangfire (one DB), Keycloak DB, Unleash DB | One Postgres instance backing four logical databases avoids four separate stateful services. Postgres is the M3.3 requirement. |
...
| Container | Tech / Version | Purpose | Why this choice |
|---|---|---|---|
| **Prometheus** | Prometheus 2.55 | Scrapes `/metrics` (15s interval); fires alert rules | Industry default for time-series metrics; rendered as self-contained chart manifests — **no** Prometheus Operator in prod ([ADR-0009](./adr/0009-hetzner-project-02-over-doks.md)). |
| **Alertmanager** | Alertmanager 0.27 | Routes Prometheus alerts to the AIOps webhook | Pairs with Prometheus; chart-rendered routing config. |
...
| **Promtail** | Promtail 3.2 (DaemonSet + namespaced `Role` in prod) | Tails every container/pod stdout → pushes to Loki | Bundled with Loki; replaces per-container log shippers. Namespace-scoped RBAC — no cluster-wide log access. |
