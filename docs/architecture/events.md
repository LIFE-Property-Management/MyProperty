...
- **`LeaseExpiringSoon` events + recurring scan.** Neither is wired yet. The query `GetLeasesExpiringSoonQuery` returns the candidate leases on demand (landlord endpoint); a Hangfire recurring scan that would push `LeaseExpiringSoonEvent` to a SignalR group is documented as a follow-up.
