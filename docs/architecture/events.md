...
- **Routing key derivation:** `IntegrationEventNaming.RoutingKey(typeof(T))` strips the `Event` suffix and converts `PascalCase` to dot-separated lower case (a dot inserted before each capital, then lowercased — note this is dot-delimited, *not* hyphenated kebab-case). `PaymentConfirmedEvent` → `payment.confirmed`.
...
- **`LeaseExpiringSoon` integration event / SignalR push.** The recurring *scan* **is** wired — the Hangfire recurring job `lease-expiring-soon` (`LeaseExpiringSoonJob`, cron `0 8 * * *`, daily 08:00 UTC) is registered in `Program.cs`. What is **not** wired is a `LeaseExpiringSoonEvent` integration event: no such event type exists, so the job publishes **no** integration event and triggers **no** SignalR push — meaning it doesn't appear in the RabbitMQ topology above. (Two *other* recurring scans are likewise scheduled — `MarkExpiredInvitesJob` hourly and `OrphanCleanupJob` daily 03:00 UTC — and they too mutate state directly and publish **no** integration event; see [`components.md`](./components.md) → Infrastructure.)
