...
The two ad-hoc jobs in production today (`SendEmailJob` with retry-and-DLQ, `ReceiptOcrJob`) plus a planned cohort of recurring scans (mark expired invites, orphan cleanup, mark overdue payments, lease-expiring) fit Hangfire's surface exactly. It uses the Postgres we already have — no new infrastructure — gives us a web dashboard for free, and runs in-process so jobs share the same `IServiceScope` as the request handlers. [ADR-0003](./adr/0003-hangfire-over-temporal.md) records the call. Temporal would buy us workflow orchestration we don't need; Quartz lacks a dashboard.
...
Second, Anthropic Claude over OpenAI / Vertex. Sonnet for receipt OCR (vision quality on structured documents, parseable JSON output, ~30 s budget); Haiku for AIOps alert triage (cheap enough that chatty alerts don't burn budget, fast enough that triage feels responsive). One vendor, one API key per environment, one billing dashboard. The vendor-concentration risk is real and accepted — both surfaces have graceful degradation paths (the AIOps webhook posts raw labels to Slack if `ANTHROPIC_API_KEY` is empty; receipt OCR shows an empty result and the user fills the form by hand).

## Why DigitalOcean (DOKS) for prod

[ADR-0004](./adr/0004-doks-over-gke-eks.md) records the cloud choice. The driver is cost — for a multi-week demo cluster running continuously, EKS' per-cluster fee (~$72/mo just for the control plane) and GKE's similar billing are hard to justify when DOKS gives a free control plane in the developer tier. DigitalOcean also wins on single-vendor billing: managed Postgres + S3-compatible Spaces in the same VPC, one credential store, no cross-cloud egress fees. Cilium is the default CNI, so `NetworkPolicy` enforcement (when toggled on) works without extra wiring.

The cost is a smaller managed-service surface (no managed Redis, no managed RabbitMQ — we self-host both) and no IRSA / Workload Identity equivalent (we ship credentials as Kubernetes Secrets, either via External Secrets Operator from GCP Secret Manager / Azure Key Vault, or via manual `kubectl create secret`). For the demo budget, the trade lands favourably.

## Why Loki, and one Grafana for everything

[ADR-0007](./adr/0007-loki-over-elk.md) records the choice. Loki's label-based indexing keeps its memory floor at ~256 MiB request, vs Elasticsearch's ~1–2 GiB JVM heap floor. On a cost-bounded DOKS node pool, that matters. Sacrifice: full-text search across all containers is slower than Elastic; labelled queries (`{container="myproperty-api"} | json | level="Error"`) are comparable. The .NET backend ships structured JSON to Loki directly via `Serilog.Sinks.Grafana.Loki` (preserving scopes + `CorrelationId`); every other container's stdout is tailed by Promtail — Docker socket SD in dev, DaemonSet + RBAC in prod.
...
Four GitHub Actions workflows — one CI per component (`backend-ci.yml`, `frontend-ci.yml`, `aiops-webhook-ci.yml`) and one CD (`cd.yml`). Each CI workflow lints, tests, builds a Docker image, scans it twice with Trivy (SARIF non-blocking for HIGH+CRITICAL, gate blocking for CRITICAL only), generates a CycloneDX SBOM, and pushes to GHCR. The CD workflow is triggered only on push to `main` / `develop`: `doctl kubeconfig` → `helm dependency update` → `helm upgrade --install --atomic --wait --timeout 10m` with the SHA tags set via `--set`, then a Discord notification. The `--atomic` flag is doing more work than it looks — it ensures a failed deploy rolls back to the previous release, and the pre-upgrade Helm Hook for EF Core migrations means a bad migration rolls back the whole release rather than half-deploying.

`--set postgres.enabled=false` in the CD workflow is what swaps the in-cluster Postgres StatefulSet for the Terraform-provisioned DO Managed PostgreSQL. The Helm chart supports both — dev uses in-cluster, prod uses managed, both via the same `myproperty-postgres` Secret.
...
- **No `GetSignedUrlAsync` on `IFileStorage`.** Local-filesystem implementation has no equivalent; M5 follow-up adds Spaces + signed URLs.
