...
| **10** | ~~**H1 — Add readiness health probe**~~ **CLOSED (Plan 4, 2026-05-16)** | ~~Required before K8s pod scheduling is meaningful. Without it, pods with broken DB connections receive live traffic. Add Postgres + Redis checks; expose `/api/v1/health/ready` distinct from the existing liveness endpoint.~~ Implemented — three-endpoint design (`/live`, `/ready`, `/diagnostics`). See `docs/operations/health-probes.md`. |
