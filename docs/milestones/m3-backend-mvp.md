...
- Per-section error handling on LandlordDashboard — ✅ done. Split into independent `OverviewSection` + `UpcomingPaymentsSection`, each owning its own loading/error/empty state. A dashboard-query failure no longer blanks the upcoming-payments table, and an upcoming-payments failure now shows its own error instead of silently masquerading as the "No upcoming payments." empty state. Tests added for both directions.
