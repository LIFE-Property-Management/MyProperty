...
> **Detailed execution journal.** The full dated record — scope, decisions, verification gates (G1..Gn), side notes, and known follow-ups for all 14 entries — is inlined at the end of this document under [Detailed entries](#detailed-entries). (Formerly a separate `m4-deployment-ops-entries.md`; merged in so M4 is one document like the other milestones.)
...
The dated execution journal for M4 — scope, decisions, verification gates (G1..Gn), side notes, and known follow-ups, in chronological order. (Merged in from the former `m4-deployment-ops-entries.md` so M4 is a single document.)
...
### 2026-05-24 — M4.8 Security hardening

**Scope.** Close M4.8 — "Trivy as CI quality gate, Key Vault/Secret Manager, distroless images." Closes the four explicit M4.4 carry-overs (NetworkPolicies, sealed-secrets/external-secrets, frontend distroless, Trivy gate flip) plus the M4.4 chiseled-migration-image gap, plus M4.3's documented Trivy blocking-gate post-M4 follow-up. Six items in one milestone; carving rationale below.

**Changes.**

*Trivy CI quality gate.*
- All three image-build workflows (`.github/workflows/{backend-ci,frontend-ci,aiops-webhook-ci}.yml`) restructured into a two-pass scan: (1) SARIF report at CRITICAL+HIGH severity, non-blocking, uploaded to the Security tab unchanged from M4.3; (2) blocking pass scoped to CRITICAL severity only, exit-code 1, fails the build. Both passes honor `trivyignores: .trivyignore` at the repo root for triaged exceptions.
- New `.trivyignore` baseline at repo root. Empty allowlist on M4.8 close; the file documents the triage workflow inline as a header comment. Format: `CVE-YYYY-NNNNN exp:YYYY-MM-DD # rationale` with the `exp:` annotation read by humans, not Trivy.
- New SBOM generation step on each workflow — `format: cyclonedx`, output `sbom-<service>.cdx.json`, uploaded as a 90-day-retention artifact. Useful for offline CVE re-scanning and downstream audit.

*Distroless image migrations.*
- `frontend/Dockerfile` runtime stage swapped from `node:20-alpine` to `gcr.io/distroless/nodejs20-debian12:nonroot`. Distroless ships only the Node runtime — no shell, no busybox, no curl. Built-in `nonroot` user (UID 65532). Standalone `server.js` entry point unchanged.
- `frontend/healthcheck.mjs` (new) — Node-based HTTP probe used by the compose-level healthcheck. Distroless has no shell to run a CMD-SHELL probe; the existing `wget --quiet --spider` path doesn't exist. Probe targets 127.0.0.1 (not localhost) to dodge the M4.2 musl IPv6 bug recurring on distroless Debian's glibc as well. K8s probes (M4.4) run `httpGet` from the kubelet and don't depend on this script — same K8s-native pattern as the chiseled backend image. `.mjs` extension is the explicit ESM marker since `frontend/package.json` has no `"type": "module"`; using `import` syntax also satisfies the `@typescript-eslint/no-require-imports` rule the rest of the frontend codebase enforces.
- `docker-compose.yml` frontend `healthcheck.test` changed to `["CMD", "/nodejs/bin/node", "/app/healthcheck.mjs"]` — invokes the bundled Node binary at distroless's standard path. CMD-form (not CMD-SHELL) because distroless has no shell.
- `backend/Dockerfile.migrations` runtime base swapped from `mcr.microsoft.com/dotnet/aspnet:10.0` (Debian, root) to `mcr.microsoft.com/dotnet/aspnet:10.0-noble-chiseled` (chiseled, UID 1654). Same base as the API image — shares the runtime layer in registries. `RUN chmod +x` removed (no shell); replaced with `COPY --chmod=755` BuildKit syntax so the executable bit survives Windows-host builds without needing shell access at build time.
- `helm/myproperty/templates/app/migration-job.yaml` — pod-level `securityContext.runAsNonRoot: true` + `runAsUser: 1654` + `fsGroup: 1654`. Container-level `readOnlyRootFilesystem: true`. Closes the migration job as a remaining PSS=restricted blocker.

*External Secrets Operator.*
- New `helm/myproperty/templates/security/secret-store.yaml` — renders either a GCP `SecretStore` or an Azure `SecretStore` based on `.Values.externalSecrets.provider`. GCP variant auths via a bootstrapped Kubernetes Secret holding a service-account JSON; Azure variant auths via service-principal credentials. Workload-identity (no bootstrap secret) is documented as the production path on both providers; the bootstrap-secret form ships because it works on DOKS without provider-side workload-identity configuration.
- New `helm/myproperty/templates/security/external-secrets.yaml` — seven `ExternalSecret` resources, one per workload-referenced Secret (`myproperty-postgres`, `myproperty-rabbitmq`, `myproperty-keycloak-admin`, `myproperty-google-oauth`, `myproperty-anthropic`, `myproperty-discord`, `myproperty-grafana`). Each maps remote keys to local Secret keys per `.Values.externalSecrets.secretRefs`. `creationPolicy: Owner` so ESO owns the Secret lifecycle. `ghcr-pull-secret` (Docker config) deliberately not under ESO — chicken-and-egg with image pulls (ESO controller image must pull before it can sync).
- `values.yaml` — new `externalSecrets` block with `enabled: false` default. The chart remains deployable on clusters without ESO installed.
- `values.yaml` — new `networkPolicies` block with `enabled: false` default + `ingressNamespace: ingress-nginx` override knob.

*NetworkPolicies.*
- New `helm/myproperty/templates/security/networkpolicies.yaml` — 13 NetworkPolicy resources gated behind `.Values.networkPolicies.enabled`:
  1. `default-deny` — denies all ingress + egress in the namespace (the foundation).
  2. `allow-dns-egress` — UDP/TCP 53 to `kube-system/k8s-app=kube-dns` for every pod.
  3. `allow-prometheus-scrape` — catch-all ingress from in-namespace Prometheus pods so scrape targets stay reachable as new workloads ship without a per-workload Prometheus stanza.
  4–13. Per-workload policies: `frontend`, `backend`, `postgres`, `redis`, `rabbitmq`, `keycloak`, `loki`, `promtail`, `aiops-webhook`, `migration`. Each carries the minimal ingress + egress required by traced traffic flows.
- External HTTPS egress (Anthropic, Google IdP, Slack/Discord) uses `ipBlock.cidr: 0.0.0.0/0` with private RFC1918 ranges in `except`, restricted to port 443. Tighter destination control is platform-layer (cloud firewall / DOKS Egress policy), not NetworkPolicy.

*Documentation.*
- New `docs/operations/security-hardening.md` — full operations doc matching the shape of `ci-cd.md`, `migrations.md`, `health-probes.md`, `nginx-ssl.md`. Sections: Overview, Trivy quality gate (with triage workflow), Distroless image runtimes (with debugging tips for no-shell environments), External Secrets Operator (with GCP + Azure activation recipes + rotation semantics + sealed-secrets fallback path), NetworkPolicy matrix (full source→destination table), Pod Security Standards (current PSS=baseline + path to restricted), Quick reference.
- `docs/operations/ci-cd.md` updated to reflect the closed Trivy carry-over and the SBOM artifact step. The "Trivy posture: non-blocking → blocking" section in `out of scope for M4.3` is struck through with a pointer to security-hardening.md.

**Decisions.**

- **CRITICAL-only blocking, not CRITICAL+HIGH.** Transitive deps in .NET + Next.js carry frequent HIGH-severity advisories that don't apply to our usage. Blocking on HIGH would generate routine false-positive failures with no actionable fix. CRITICAL-only catches the genuinely urgent findings while leaving HIGH visible in the SARIF report for scheduled triage. The `.trivyignore` covers both severity tiers — adding a HIGH entry there suppresses both the SARIF report and a future flip to HIGH-blocking, when the triage backlog has been worked off.
- **Two-pass workflow over single configurable pass.** Could have used a single Trivy step with `severity: CRITICAL,HIGH` + `exit-code: '1'` and relied on `.trivyignore` to silence HIGH at the gate. Rejected: silencing HIGH at the gate also silences it in the SARIF report (Security tab), defeating the visibility argument. Two passes is more YAML but the failure modes are distinct (block on CRITICAL, surface HIGH).
- **CycloneDX over SPDX for SBOMs.** Both formats are first-class in Trivy. CycloneDX has wider tooling support for downstream consumers (dependency-track, etc.). SPDX is the OpenChain standard but its tooling ecosystem is smaller.
- **ESO opt-in (`enabled: false` default), not on-by-default.** The chart must deploy on clusters where ESO is not installed (school graders, fresh clones, kind/k3d). Default off + values flag to enable matches the precedent set by `values.networkPolicies.enabled` (same milestone). The manual `kubectl create secret` runbook in `docs/operations/k8s-deployment.md` remains the no-ESO bootstrap path.
- **GCP + Azure SecretStores in one template via `provider` switch.** One template renders one of the two providers based on `.Values.externalSecrets.provider`. Trade-off: the template carries both code paths even when only one is used. Cost: ~30 lines of YAML. Benefit: switching providers is a one-flag change with no template re-author.
- **Bootstrap secret for SecretStore auth, not workload-identity.** Both GCP Workload Identity (GKE) and AKS Workload Identity are the production-grade paths. They require platform-side configuration (GKE-only on GCP, AKS-only on Azure) that is out of scope for this milestone (and not available on DOKS at all). The bootstrap-secret form works on any K8s distribution and is documented as the school-demo path; production migration to workload-identity is a one-template edit + the platform-side federated-credential setup.
- **ESO `creationPolicy: Owner`.** ESO is the source of truth for the Secret resource — deleting the ExternalSecret deletes the Secret. The alternative (`Merge`) lets a pre-existing manually-created Secret keep its other keys while ESO writes its own. We don't have a mixed-ownership case; Owner is the simpler contract.
- **One `ExternalSecret` per Kubernetes Secret, not one Secret-per-key.** Could have split `myproperty-postgres` into three ExternalSecrets (`-user`, `-password`, `-db`). Rejected: it triples the resource count without changing the operational picture — all three rotate together (any rotation either keeps the existing workload working or breaks it; partial rotation is not a sane operational state).
- **`ghcr-pull-secret` stays manually bootstrapped.** ESO controller's own image must pull from a registry before it can sync any Secret. Pull-secret-via-ESO is the canonical chicken-and-egg failure. Documented in the operations doc + values.yaml comment.
- **Sealed-secrets documented as the fallback, not as a parallel mechanism.** For school-demo environments where neither GCP nor Azure access is available, sealed-secrets is operationally equivalent (encrypted material in Git, decrypted in-cluster). M4.8 deliverable text explicitly names Key Vault/Secret Manager; sealed-secrets deviates from that text the same way Loki+Promtail deviated from "ELK Stack" in M4.5. Documenting the path lets a teammate without cloud access still close the demo flow.
- **NetworkPolicies opt-in (`enabled: false` default), not on-by-default.** Three reasons: (1) CNIs without NetworkPolicy enforcement (plain Flannel) install the resources without applying them — silent failure mode; default-off forces a deliberate enablement decision per cluster. (2) A misconfigured allow-rule on a cluster that does enforce will produce hard-to-debug "service can't reach service" failures during cluster bringup. (3) DOKS specifically ships Cilium and will enforce — and the bringup sequence has enough moving parts (DNS for image pulls, ingress controller install, cert-manager install, kube-prometheus-stack CRDs) that adding NetworkPolicy enforcement to the bringup itself increases failure modes. Activation post-bringup is one `--set networkPolicies.enabled=true` flag.
- **Default-deny + targeted allow over default-allow + targeted deny.** NetworkPolicy's API only supports the former — `policyTypes: [Ingress, Egress]` + empty `podSelector: {}` means "deny everything for everyone selected." But the deliberate choice to ship the default-deny resource at all (vs leaving the namespace unconstrained) is what makes the targeted policies meaningful. Without default-deny, an unselected workload has implicit ingress + egress — the per-workload policies layer would be advisory rather than enforced.
- **External HTTPS egress on `:443` only, not per-destination.** The proper way to restrict outbound to specific hostnames is a CNI feature (Cilium FQDN policies, Calico DNS policies) or a service-mesh egress gateway (Istio). Plain NetworkPolicy only matches IPs + CIDRs; "allow egress to api.anthropic.com" is structurally impossible. The `cidr: 0.0.0.0/0` + private-CIDR `except` clause is the standard pragmatic workaround — egress is restricted to public IPs on port 443, with finer destination control deferred to the platform layer (DOKS Egress Firewall, etc.).
- **`COPY --chmod=755` on the chiseled migration image.** The previous `RUN chmod +x` was a safety net for Windows-host builds (NTFS has no Unix executable bit; Docker BuildKit applies default 0644 to copied files). Chiseled has no shell so RUN is unavailable. BuildKit's `--chmod` flag on COPY sets the mode at copy time and works on both Windows and Linux hosts. Modern Docker (24+) defaults to BuildKit; CI uses buildx (BuildKit) explicitly.

**Verification.**
- **G1** (workflow YAML): `yaml.safe_load` parses all three modified workflows. Trivy step count went 1 → 4 per workflow (SARIF + SARIF upload + gate + SBOM gen + SBOM upload = 5 steps; previously 2).
- **G2** (helm lint): `helm lint helm/myproperty` exits 0 with the new templates in place (subject to ESO + NetworkPolicy CRDs being known to kubeconform's schema cache).
- **G3** (helm template defaults): `helm template helm/myproperty | grep -c 'kind: ExternalSecret'` returns 0 — defaults render zero ESO resources, as designed. Same for `kind: NetworkPolicy`.
- **G4** (helm template ESO+NP enabled): `helm template helm/myproperty --set externalSecrets.enabled=true --set externalSecrets.gcp.projectId=demo --set networkPolicies.enabled=true | grep -cE 'kind: (ExternalSecret|NetworkPolicy|SecretStore)'` returns 21 (7 ExternalSecret + 13 NetworkPolicy + 1 SecretStore).
- **G5** (compose syntax): `docker compose config --quiet` exits 0 after the frontend healthcheck swap.
- **G6** (`.trivyignore` parses): the file is a plain-text allowlist; format is `CVE-YYYY-NNNNN` per line. Empty baseline is valid.
- **G7** (docs cross-references): `security-hardening.md` references `ci-cd.md`, `k8s-deployment.md`, `migrations.md`, `health-probes.md`, `nginx-ssl.md` — all exist in `docs/operations/`. The strikethrough section in `ci-cd.md` references `security-hardening.md`.

**Live verification deferred to first deploy.** Same gating as M4.4. Server-side validation (ESO controller actually syncing, NetworkPolicy actually enforcing under Cilium, Trivy gate actually failing on a seeded CRITICAL) is exercised on the first post-merge CI run + the first cluster deploy. The chart is complete-by-construction up to `helm template | kubectl --dry-run=client apply -f -`.

**Side notes / deviations.**

- **Distroless nodejs digest pinning gap.** New Dockerfile base reference is `gcr.io/distroless/nodejs20-debian12:nonroot` (tag-only) — first post-merge Dependabot run will append the `@sha256:...` digest. Likewise for `mcr.microsoft.com/dotnet/aspnet:10.0-noble-chiseled` on `Dockerfile.migrations`. Breaks the M4.3 supply-chain convention temporarily; manual `docker pull ... && docker inspect --format '{{index .RepoDigests 0}}' ...` lookup + pin is acceptable as a stop-gap. Documented in each Dockerfile header.
- **Distroless Python deferred.** `gcr.io/distroless/python3-debian12` was considered for the aiops-webhook image but the distroless Python image was unmaintained as of 2024-Q4. Chainguard's `cgr.dev/chainguard/python` (Wolfi-based) is the actively-maintained alternative; promotion is an M5 follow-up if the attack-surface reduction is judged worth a third-party registry dependency. The current `python:3.14-slim` + non-root `aiops` user closes most of the hardening surface without the registry switch.
- **PSS=restricted gate still blocked.** M4.8 closes the migration-job exemption (now non-root chiseled). Promtail remains rooted because reading kubelet log files at `/var/log/pods/*` requires root or CAP_DAC_READ_SEARCH. Three escape routes documented in security-hardening.md (Vector replacement, log-driven sidecar pattern, fluent-bit with the same constraints). None are M4 critical-path.
- **NetworkPolicy schemas for `kubeconform`.** Native K8s NetworkPolicy schemas are bundled in `kubeconform`'s default schema-location. ESO CRDs (`SecretStore`, `ExternalSecret`) are not — verification commands need the `datreeio/CRDs-catalog` schema-location flag, same as the kube-prometheus-stack CRD verification from M4.4.
- **ESO ClusterSecretStore not shipped.** A `SecretStore` is namespace-scoped (lifecycle tied to the release), which is what we want. `ClusterSecretStore` is cross-namespace and would be a cluster-wide resource — appropriate for a fleet of helm releases sharing one cloud backend, not for the single-tenant `myproperty` release.
- **AIOps webhook re-rooted under PSS=restricted concerns.** The aiops-webhook image runs as non-root `aiops` user; the deployment has `readOnlyRootFilesystem: true`. Already PSS=restricted-compatible. The blocking workload for the namespace flip is promtail, not aiops-webhook.

**Known follow-ups (out of scope for M4.8).**

- **Trivy HIGH-severity blocking.** Once the .trivyignore baseline has triaged the HIGH-advisory backlog, flip the SARIF pass from `exit-code: 0` to `exit-code: 1` (or add a third pass scoped to HIGH+blocking). Owned by a post-M4 cleanup sprint.
- **Workload Identity for ESO auth.** Replace the bootstrap-secret + service-account-JSON path with GKE Workload Identity (GCP) or AKS Workload Identity (Azure). Eliminates the chicken-and-egg bootstrap. Requires platform-side federated-credential setup; not available on DOKS.
- **Sealed-secrets actually shipped as a Helm template.** Currently sealed-secrets is documented in the operations doc as a fallback path; if the project goes the sealed-secrets route in production, ship the SealedSecret manifests under `templates/security/` alongside the ESO templates with a `secretsBackend: ` value switch.
- **Promtail → Vector or similar non-root log shipper.** Closes the remaining PSS=restricted gap. Vector + Loki sink is the lowest-friction path; requires Loki write-config + a Vector ConfigMap rewrite.
- **CNI FQDN policies for egress destination control.** Replace the `cidr: 0.0.0.0/0` + private-CIDR-except egress rules with Cilium FQDN policies (`api.anthropic.com`, `accounts.google.com`, `hooks.slack.com`, etc.). Requires Cilium FQDN feature gate and Cilium-only NetworkPolicy CRD; the resulting policies don't survive a CNI swap. Defer until egress destination control becomes a deliberate threat-model decision.
- **CIS Kubernetes Benchmark + Polaris scan in CI.** Bring k8s-bench or Polaris into the chart-template stage to surface PSS / RBAC / NetworkPolicy / ServiceAccount findings at PR time, not at deploy time. Same shape as Trivy's image scan but at the manifest layer.
- **Image signing (cosign / sigstore).** The supply-chain story today: GHCR + SHA tag + Dependabot digest pinning + Trivy CVE scan + SBOM artifact. Cosign signing + Rekor transparency-log entry closes the remaining "what proves this image was built by our CI" gap. Image-policy admission (Sigstore's policy-controller or Kyverno) enforces it at deploy time. Post-M4 supply-chain work.
- **CD workflow's `helm upgrade` with `--values` overlays per provider.** When the secret-manager backend goes live in a real environment, the CD workflow's `helm upgrade --install` step gains a `--set externalSecrets.enabled=true --set externalSecrets.provider=...` chain. Today's CD workflow defaults to manual-bootstrap because the secrets-bootstrap path is documented but unprovisioned.
