...
// the tone tests assert those rather than just the visible label. The flow is
// two steps since the signature/ID step was removed (D6).

describe("StepIndicator", () => {
  it("renders both step titles", () => {
  ...
  it("styles past steps as done, the current step as current", () => {
    render(<StepIndicator current={1} />);
  ...
  it("styles a later step as upcoming when not yet reached", () => {
    render(<StepIndicator current={0} />);

    // Step 2 (index 1) is ahead → surface background, not filled.
